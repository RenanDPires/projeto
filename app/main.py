...
def _build_sweep_axis(min_value: float, max_value: float, n_points: int, log_scale: bool) -> np.ndarray:
    """Gera eixo de varredura com escala linear ou logarítmica."""
    if log_scale:
        values = np.geomspace(float(min_value), float(max_value), int(n_points))
    else:
        values = np.linspace(float(min_value), float(max_value), int(n_points))
    return _limit_sweep_points(values, max_points=int(n_points))


def _apply_log_x_axis(figures: list[go.Figure], log_scale: bool) -> None:
    """Aplica escala logarítmica no eixo X para uma lista de figuras."""
    if not log_scale:
        return
...
def _show_assessment_q1_tab() -> None:
    ...
    if not enabled_materials:
        ...
        if freq_max <= freq_min:
            ...
            x_freq = _build_sweep_axis(freq_min, freq_max, n_points, log_scale_x)
            x_current = _build_sweep_axis(i_min, i_max, n_points, log_scale_x)
            ...
            _apply_log_x_axis([fig_freq, fig_current], log_scale_x)
...
def _show_assessment_q3_tab() -> None:
    ...
    if not q3_selected_materials:
        ...
        if q3_freq_max <= q3_freq_min:
            ...
            q3_x_freq = _build_sweep_axis(q3_freq_min, q3_freq_max, q3_n_points, q3_log_x)
            q3_x_current = _build_sweep_axis(q3_i_min, q3_i_max, q3_n_points, q3_log_x)
            ...
            _apply_log_x_axis([q3_fig_freq, q3_fig_current], q3_log_x)
...
def show_assessment_01_page() -> None:
    ...
    with tab_q1:
        render_q1_tab(
            plotly_chart_with_csv=_plotly_chart_with_csv,
            build_sweep_axis=_build_sweep_axis,
            apply_log_x_axis=_apply_log_x_axis,
        )
    ...
    with tab_q3:
        render_q3_tab(
            plotly_chart_with_csv=_plotly_chart_with_csv,
            apply_im_to_conductors=_apply_im_to_conductors,
            build_sweep_axis=_build_sweep_axis,
            apply_log_x_axis=_apply_log_x_axis,
        )

    with tab_q4:
        render_q4_tab(plotly_chart_with_csv=_plotly_chart_with_csv)
